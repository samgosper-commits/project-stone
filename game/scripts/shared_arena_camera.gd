...
func _ready()->void:current=true
func _process(delta:float)->void:
 players.assign(get_tree().get_nodes_in_group("arena_players"))
 if players.is_empty():return
 var centre:Vector3=Vector3.ZERO
 for player in players:centre+=player.global_position
 centre/=float(players.size())
 var max_spread:float=0.0
 for player in players:max_spread=maxf(max_spread,player.global_position.distance_to(centre))
 var height:float=clampf(min_height+max_spread*spread_zoom_factor,min_height,max_height)
 var pitch:float=deg_to_rad(pitch_degrees)
 var desired:Vector3=centre+Vector3(0.0,sin(pitch)*height,cos(pitch)*base_distance+max_spread*0.35)
